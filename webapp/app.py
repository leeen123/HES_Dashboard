...
def index():
    if request.method == 'POST':
        ...
        # 发送数据到 shap_plot.php
        shap_plot_php_url = '../webapp/templates/shap_plot.php'
        shap_response = requests.post(shap_plot_php_url, data={
            'shap_plot': 'static/shap_plot.png',
            'variable_data': variable_data,
            'interaction_variable_data': interaction_variable_data
        })
        shap_php_result = shap_response.text  # PHP 脚本返回的响应
...
def test():
    if request.method == 'POST':
        ...
        # 发送数据到 try.php
        try_php_url = '../webapp/templates/try.php'
        try_response = requests.post(try_php_url, data={
            'variable_data': k_mean_analy_data,
            'cluster_data': cluster_data,
            'income_category_data': income_category_data
        })
        try_php_result = try_response.text  # PHP 脚本返回的响应
